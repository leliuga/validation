...
// ValidationValuer Interface for transforming values to validation proxies.
//
...
type ValidationValuer func(any) (any, bool)

// Default tranformation function to convert sql.driver.Valuer
// based values
func sqlValueValuer(orig any) (any, bool) {
...
func ToAnySliceFromMapKeys[M ~map[K]V, K comparable, V string](m M) []any {
	r := make([]any, 0, len(m))
	for k := range m {
		r = append(r, k)
	}
