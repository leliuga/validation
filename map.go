...
//
//	validation.Map(
//	    validation.Key("Type", validation.Required),
...
// OptionalKey specifies an optional map key and the corresponding validation rules.
// the rule will be ignored if the key is missing.
func OptionalKey(key any, rules ...Rule) *KeyRules {
	return &KeyRules{
		key:      key,
		rules:    rules,
		optional: true,
